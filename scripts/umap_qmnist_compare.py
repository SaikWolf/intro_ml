...
def main():
    ...
    labels = [cls_label[x[0]] for x in df['labels'].values]
    df['labels'] = labels
    ...
    ae_model = AutoEncoder_v0.load_from_checkpoint('lightning_logs/ae_qmnist/stage_3/model.ckpt')
    cls_model = Classifier_v0.load_from_checkpoint('lightning_logs/cls_qmnist/stage_3/model.ckpt')
    ...
    df_ae = make_df(ds(ae_features,predicted_lbl[:,0]))
    df_cl = make_df(ds(cf_features,predicted_lbl[:,0]))
    del predicted_lbl, ae_features, cf_features

    labels = [cls_label[x] for x in df_ae['labels'].values.reshape(nobs,)]
    df_ae['labels'] = labels
    df_cl['labels'] = labels
    del labels
    # print(df_ae)
