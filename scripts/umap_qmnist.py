...
def main():
    ...
    labels = [str(x[0]) for x in df['labels'].values]
    df['labels'] = labels
    print("Flattened dataset shape:",digits.shape)
    # https://umap-learn.readthedocs.io/en/latest/basic_usage.html#digits-data
    reducer = UMAP(n_neighbors=15,n_components=2,metric='euclidean',min_dist=0.1,verbose=True,low_memory=True,random_state=rng.integers(np.iinfo(np.int32).max))
    ...
    sb.scatterplot(df,x="embed0",y="embed1",hue="labels",hue_order=[str(x) for x in range(10)],ax=ax)
    plt.show()
